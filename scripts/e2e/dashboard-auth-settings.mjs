    ...
    if (role === 'admin') {
      ...
      const costs = installations.flatMap(p => [['top-e2e-a', 10], ['top-e2e-b', 3], ['top-e2e-c', 2]]
        .map(([model, cost]) => JSON.stringify({ ...p, event_id: randomUUID(), signal: 'log',
          raw_json: JSON.stringify({ type: 'llm_call', payload: { model, cost_usd: cost, duration_ms: cost, tokens: { input: 1, output: 0, cache_read: 0, cache_create: 0 } } }) })));
      ...
      const agentEvents = installations.flatMap(p => [
        ...['a', 'b', 'c'].map(agent => ({ agent, teams: [sessionTeams[0]] })),
        ...['x', 'y', null].map(agent => ({ agent, teams: sessionTeams.slice(1) })),
      ].map(({ agent, teams }) => JSON.stringify({
      ...
      const anomalyEvents = installations.flatMap(p => sessionTeams.flatMap((team, index) => [0, 1].map(day => JSON.stringify({
        ...p, event_id: randomUUID(), ts: p.ts - day * 86400, signal: 'log', team_ids_as_of: [team],
        raw_json: JSON.stringify({ type: 'llm_call', payload: { model: 'anomaly-top-e2e',
      ...
      try {
        ...
        const agentTop = await ownerPage.evaluate(async () => {
          const { request } = await import('/src/api/client.ts');
          const { series } = await import('/src/widgets/model.ts');
          const response = await request('/query', { method: 'POST', body: JSON.stringify({
            from: 'now-1d', to: 'now', filters: { models: ['agent-top-e2e'] },
            queries: [{ ref_id: 'A', metric_id: 'subagent_activity', group_by: ['team'], frame_type: 'table', limit: 1 }],
          }) });
          return series(response.results.A);
        });
        ...
        const onboardingTop = await ownerPage.evaluate(async () => {
          const { request } = await import('/src/api/client.ts');
          const { series } = await import('/src/widgets/model.ts');
          const response = await request('/query', { method: 'POST', body: JSON.stringify({
            from: 'now-1d', to: 'now', filters: { models: ['session-top-e2e'] },
            queries: [{ ref_id: 'A', metric_id: 'onboarding_ttfu', group_by: ['team'], frame_type: 'table', limit: 1 }],
          }) });
          return series(response.results.A);
        });
        ...
        const anomalyTop = await ownerPage.evaluate(async () => {
          const { request } = await import('/src/api/client.ts');
          const { series } = await import('/src/widgets/model.ts');
          const response = await request('/query', { method: 'POST', body: JSON.stringify({
            from: 'now-1d', to: 'now', filters: { models: ['anomaly-top-e2e'] },
            queries: [{ ref_id: 'A', metric_id: 'cost_anomaly', group_by: ['team'], frame_type: 'scalar', limit: 1,
              params: { moving_avg_days: 1 } }],
          }) });
          return series(response.results.A);
        });
