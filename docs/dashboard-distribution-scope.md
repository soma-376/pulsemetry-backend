...
상태: 사용자 범위 결정 대기. 아래는 현재 구현의 사실과 제안이며, 미지원 항목을 수용 완료로 처리하지 않는다.
...
현재 권고는 1번이다. 사용자에게 두 선택지를 질문했으며 응답 전에는 승인으로 간주하지 않는다. UI 감사 입력·비교/잔존율 라벨 결함은 frontend acdc626에서 수정하고 실제 E2E로 검증했다. 해당 검증의 한계는 dashboard-acceptance-matrix.md를 참조한다.
