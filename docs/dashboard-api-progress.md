...
4. 46개 시나리오: 카탈로그·상세는 구현했다. 실행 파라미터 서버 검증·조회 시 가용성·실측 기반 findings는 남아 있다.
...
- `DashboardScenarioInputs.prepare`를 추가했다. HTTP 실행 API와 큐·워커에는 아직 연결하지 않았다. 따라서 SCN-RUN 구현 완료나 실행 입력의 HTTP 수용 검증을 뜻하지 않는다. 구현된 operation 수는 13개로 유지한다.
