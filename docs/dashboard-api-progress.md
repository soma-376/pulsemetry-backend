...
2. 계약 기반 파생 지표 검토: 약정 소진율도 연결했다. 계약 ID를 지정한 약정 소진율은 366일 초과 조회를 허용한다. 일반 지표·미지정 계약은 기존 제한을 유지한다.
...
## 마지막 세션 이벤트 상위 N 계약 보완

- `session_last_event` table/scalar/timeseries의 상위 N을 연결했다. 현재 기간 세션 수로 상위 팀을 선택하며 비교 기간에 같은 선택을 적용한다.
- 기타의 설치·제품·세션을 합쳐 마지막 로그를 다시 선택한다. 기존 시각·sequence·event_id 순서와 오류 유형 판정을 유지한다. 요청 limit는 팀 수에 적용하고 파생 last_event 유형은 별도로 보존한다.
- 구현 `1a61019`, dashboard 테스트 257건·JAR 검증 통과, 실패·오류·skip 0건. 세 형식에서 기타 최종 오류 5건, 비교 프롬프트 5건, 현재 없는 비교 유형의 보존을 검증했다. 기존 유형별 소집단·순번·제품 구분 테스트도 통과했다.

### 마지막 세션 이벤트 상위 N E2E 결과

- 구현 `1a61019`와 frontend `52f7cb10017c6ba6120f51f2e158ff329d14bff0`의 E2E 통과. 실제 owner 로그인 후 API 클라이언트·위젯 변환에서 다른 팀에 걸친 동일 세션의 최종 오류 5건을 확인했다.
- 실제 수집·기존 42개 시나리오 경로도 통과했고 `unexpectedOrUnimplementedResponses=[]`다. frontend 소스는 변경하지 않았다. 이번 추가 검증은 API·위젯 데이터 계약이며 새 화면의 시각 검증은 아니다.
- 일반 그룹 상위 N은 50개 연결·잔존율 1개 미연결이다. 전체 형식·장기 계약·운영 복구 수용 조건은 별도 남아 있다.

## 잔존율 상위 N 계약 보완

- `onboarding_retention` table/scalar의 상위 N을 연결했다. 완료된 코호트·주차 셀의 설치 수로 가중한 잔존율(분자 합계/분모 합계)로 상위 팀을 선택한다. 미완료 셀은 순위에서 제외하며 숨겨진 수치는 사용하지 않는다. 비교 기간은 같은 선택을 적용한다.
- 기타의 설치별 최초 관측과 활동 주차 합집합으로 코호트를 다시 구성한다. 중복 소속 설치를 한 번만 세고 파생 코호트·주차는 팀 limit로 잘라내지 않는다. 기존 미완료·미래 주차 null, 과거 이력, 코호트별 소집단 처리를 유지한다.
- 구현 `922a5d7`, dashboard 테스트 258건·JAR 검증 통과. 새 테스트는 table/scalar에서 기타 코호트 분모 5개, 주차별 1·1·0을 검증했다. 기존 비교 소집단·미완료·미래 주차 테스트도 통과했다.
- 일반 group_by 허용 지표 51개 모두 상위 N 분기가 연결됐다. group_by 금지 2개와 계약별 내부 결과 제한은 별도다. 모든 지표·차원·기간·권한 조합의 수용 검증이 끝났다는 의미는 아니다.

### 상위 N 전체 보완 후 전체 빌드

- `922a5d7`에서 `./gradlew build --rerun-tasks` 통과. 전체 1,065건(dashboard 258건 포함), 실패·오류·skip 0건. 58개 작업을 캐시 재사용 없이 실행했다. 로그 `/tmp/proj156-top-complete-build.log`.

### 잔존율 상위 N E2E 결과

- 구현 `922a5d7`와 frontend `52f7cb10017c6ba6120f51f2e158ff329d14bff0`의 E2E 통과. 실제 owner 로그인 후 공통 API 클라이언트에서 기타 코호트 설치 분모 5개를 확인했다. 완료 주차의 잔존율 재계산은 DB 통합 테스트가 담당한다.
- 실제 수집·기존 42개 시나리오 경로도 통과했고 `unexpectedOrUnimplementedResponses=[]`다. frontend 소스는 변경하지 않았다. 이번 추가 검증은 API 데이터 계약이며 새 화면의 시각 검증은 아니다.
- 일반 그룹 상위 N의 명시적 지원 분기는 모두 연결됐다. 다음은 남은 frame 형식·장기 계약·개인정보 감사 흐름·운영 복구 등 전체 수용 조건 대조다.

## 계약 지정 약정 소진율의 장기 조회

- 공통 366일 제한이 장기 약정 조회를 막던 부분을 보완했다. 요청의 모든 쿼리가 contract_id를 명시한 `contract_commitment_burn`인 경우에만 긴 기간을 허용한다. 계약 미지정·일반 지표·혼합 요청은 366일 제한을 유지한다.
- 기존 owner 인가·전사 범위·계약 배정 및 유효 기간·통화·할인 규칙, DB 응답 시간·행 수·크기·시계열 포인트 제한은 유지한다. 계약 기간을 자동 선택하지 않으며 요청 from/to 안에서 계산한다.
- 구현 `575398c`, dashboard 테스트 259건·JAR 검증 통과, 실패·오류·skip 0건. 2024~2026년의 비용 150과 약정액 1000, 소진율 0.15를 table/scalar로 검증했다. 제한을 우회하는 혼합 요청도 422로 거부한다.

### 장기 약정 조회 E2E 결과

- 구현 `575398c`와 frontend `52f7cb10017c6ba6120f51f2e158ff329d14bff0`의 E2E 통과. 실제 owner 로그인 후 공통 API 클라이언트로 계약 ID 지정 730일 요청이 성공하고 비용/약정액=소진율이 성립함을 확인했다. 실제 오래된 비용의 포함은 DB 통합 테스트가 담당한다.
- 실제 수집·기존 42개 시나리오 경로도 통과했고 `unexpectedOrUnimplementedResponses=[]`다. frontend 소스는 변경하지 않았다. 새 UI 날짜 선택이나 시각 검증이 아닌 API 연동 검증이다.

## 잔존율 코호트별 주간 시계열

- `onboarding_retention`의 timeseries를 연결했다. 요청 시간대의 주 시작일을 시간축으로 삼아 코호트별 프레임을 반환한다. interval 생략 시 1w, 명시 시 1w만 허용한다. 다른 간격으로 주간 잔존율을 복제하지 않는다.
- 비교 값은 상대 코호트·주차에 맞춰 정렬한다. 기존 미완료 주차 null과 코호트 소집단 마스킹을 유지하며 상위 N은 팀 수에 적용한다. table/scalar의 코호트·주차별 프레임은 유지한다.
- 구현 `e96632d`, dashboard 테스트 260건·JAR 검증 통과, 실패·오류·skip 0건. 주 시작일과 현재 [1,0.6,0]·비교 [1,0,0] 정렬, 1d 거부를 검증했다. 기존 코호트·미래 주차·소집단 테스트도 통과했다.

### 잔존율 시계열 E2E 결과

- 구현 `e96632d`와 frontend `52f7cb10017c6ba6120f51f2e158ff329d14bff0`의 E2E 통과. 실제 owner 로그인 후 공통 API 클라이언트로 interval 생략 시 주간 시간 필드와 코호트 설치 분모 5개를 확인했다. 완료 주차와 비교 수치 정렬은 DB 통합 테스트가 담당한다.
- 실제 수집·기존 42개 시나리오 경로도 통과했고 `unexpectedOrUnimplementedResponses=[]`다. frontend 소스는 변경하지 않았다. 이번 추가 검증은 API 데이터 계약이며 새 시계열 화면의 시각 검증은 아니다.

## W3.3 벤더 불일치 주소 도메인 표

- `vendor_account_mismatch`의 명시적 table 요청에 installation_id·vendor_email·registered_email 문자열 열을 연결했다. 마지막 비어 있지 않은 관측 주소와 등록 주소를 대소문자 무시로 비교하며 불일치 설치만 포함한다. scalar/timeseries는 기존 개수 집계를 유지한다.
- 기존 owner·X-Audit-Reason 검증과 감사 기록을 유지한다. 주소는 로컬 부분을 제거한 ***@도메인으로 제공하고 잘못된 주소는 null로 둔다. 비교 요청은 설치 합집합과 비교 주소 열로 표현한다.
- 현재·비교 관측 인원 중 5명 미만이면 식별자·주소와 행 수를 단일 null 행으로 숨긴다. 비마스킹 결과가 limit를 초과하면 422로 거부하며 부분 목록으로 자르지 않는다.
- 구현 `58ba9f3`, dashboard 테스트 261건·JAR 검증 통과, 실패·오류·skip 0건. 감사 없는 요청 거부, 5개 설치의 도메인 전용 표, 4명 소집단 식별자·주소·행 수 마스킹을 검증했다.
...
- `86a2a29`와 frontend `52f7cb1` E2E 통과. 실제 수집 → S4-8 실행에서 p90 120,000ms·1분 초과 판정과 evidence.dimensions.team, 지표 프레임의 동일 팀 라벨을 확인했다. 기존 결과 화면 이동·판정 제목 확인도 통과했다.
- dashboard 테스트 266건(실패·오류·skip 0)·bootJar와 기존 42개 시나리오·프로세스 재기동·ClickHouse 장애 복구 E2E가 통과했다. unexpectedOrUnimplementedResponses=[]이며 알려진 주소 표 감사 누락은 별도 knownUiGaps에 유지한다. 로그 /tmp/proj156-gate-team-e2e.log.
- 프런트엔드 소스는 수정하지 않았다. 팀별 수치·마스킹은 DB 검증, 실제 클라이언트 팀 라벨 수신은 E2E 검증이다. 모든 팀의 결과 화면 시각 검증이나 전체 시나리오 의미 대조 완료를 뜻하지 않는다.

## 전후 비교 5개 시나리오 의미 대조 완료

- S4-4·S5-6·S6-3·S8-6·S8-7의 기준일 경계, 달력 주, 선택 범위, p50/건수/비율, 소집단·미관측·미완료 기간 처리와 판정 한계를 대조했다. 새 런타임 결함은 발견하지 못했다.
- docs/dashboard-comparison-acceptance.md에 시나리오별 의미·소스·기존 테스트 근거를 정리했다. 문서에 연결한 테스트 10건이 실제 소스에 존재하고 최신 JUnit XML에서 실패·오류·skip 없이 통과했는지 확인했다.
- 이번 작업은 수용 근거 문서만 변경했다. 런타임과 frontend는 변경하지 않았고, 직전 86a2a29의 dashboard 266건/E2E를 중복 실행하지 않았다. 일반 P3 시작 폼 감사와 비교 화면의 시각 수용, 나머지 시나리오 대조·distribution 정의는 여전히 남아 있다.

## 나머지 시나리오 의미 검토와 전체 목록 완료

- docs/dashboard-scenario-semantics.md에 나머지 37개 실행 시나리오의 관측값·판정 조건·해석 한계·대표 DB 테스트를 정리했다. 전후 비교 5개와 실행 불가 4개를 합쳐 카탈로그 46개 ID와 누락 없이 일치함을 확인했다.
- 대표 테스트 37개 이름이 실제 DashboardAuthTest 소스에 존재하고 최신 JUnit XML에서 실패·오류·skip 없이 통과했는지 확인했다. DashboardThresholdFindings의 20개 규칙과 전용 평가기·입력·실행 경로를 대조했다. 새 런타임 수정 근거는 발견하지 못했다.
- 시나리오 관측 의미의 소스 대조 목록은 완료했다. 일반 UI 감사 전달(CSV/P3/주소 표), 표·비교·잔존율 화면 수용, 미지원 distribution의 정의는 남아 있다. 공격 탐지·인과 효과·재무 성과를 새로 구현했다고 해석하지 않는다.
- 문서 변경만 수행했고 frontend·런타임·테스트 코드는 변경하지 않았다. 최신 86a2a29의 dashboard 266건·E2E 이후 동일 검증을 재실행하지 않았다.

## distribution 지원 범위 결정안

- docs/dashboard-distribution-scope.md에 지원 8개 지표의 실제 분위수·버킷·곡선 필드와 미지원 45개 목록을 정리했다. 런타임 지원 집합과 기존 계약 추적표의 53개 행이 일치하는지 확인했다.
- 미지원 지표의 기본 집계와 distribution 형식 미지원을 구분했다. 범주별 table을 숫자 히스토그램과 혼동하지 않으며, 추가 지원에는 표본·단위·버킷·누락·마스킹·비교 정의가 필요함을 명시했다.
- 사용자에게 현재 8개를 티켓 지원 범위로 확정할지, 추가 지표와 분포 표본을 지정할지 질문했다. 응답 대기이며 현재 범위를 수용 승인한 것으로 처리하지 않는다.
- 문서만 변경했고 런타임·frontend·테스트는 변경하지 않았다. 기존 dashboard 266건/E2E를 중복 실행하지 않았다. UI 감사 전달·시각 수용은 이 범위 결정과 별도로 남아 있다.

## S4-4 비교 결과 표 DOM·시각 검증
