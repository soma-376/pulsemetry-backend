...
상태: 진행 중. 전체 22개 operation·53개 지표·50개 시나리오 구현과 최종 E2E는 아직 완료되지 않았다.
...
- 기존 security/enrollment 테스트 및 전체 `./gradlew build` 통과: 현재 테스트 결과 888건, 실패·오류·skip 0건.
...
1. QRY 확장: 남은 3개 지표의 이벤트/비율/분포/코호트 계산, 비율 분모, 상위 N의 `__other__` 집계와 품질 캡션을 연결한다. 현재 50개 지표의 공통 조회·비교·CSV·마스킹 경로를 재사용한다.
...
4. 50개 시나리오: 카탈로그·파라미터 검증·조회 시 가용성·실측 기반 findings.
...
- QRY의 나머지 3개 지표 및 다른 지표의 distribution 형식, 설치·세션·시나리오·실행·저장 API는 미구현이다. 지표 결과나 가용성을 임의로 만들어 반환하지 않는다.
...
- `availability`는 OpenAPI 정의대로 현 스키마에서의 산출 가능성이다. 데이터가 없는 기간, 관리자 권한, 집계 코드의 구현 여부를 이 값으로 표현하지 않는다. 현재 50개 지표는 QRY에서 관측·품질·권한을 판정한다. 나머지 지표와 시나리오 실행의 판정은 계속 구현해야 한다.
- 부분 측정 지표는 서브에이전트 활동, API 오류율, 훅 실행·차단, 모델 거부 5개다. 미보존 이벤트와 tracing 전제를 caveat에 포함했다.
- `min_group_size=5`는 구현된 50개 집계의 마스킹에 적용했고, 허용되지 않은 group_by는 전체 요청을 400으로 거부한다. 아직 연결되지 않은 집계까지 구현됐다는 뜻은 아니다.
...
- 실제 frontend owner/admin 위젯 변환 검증 데이터: 재시도 5/15=1/3, 429 호출 5건. 기존 지표를 포함해 50개를 검증하며 전체 ingest E2E는 별도 남아 있다.
...
- 실제 frontend owner/admin의 팀·도구별 검증 데이터: 자동 결정 비율 10/20=0.5, 거절 수 5. 기존 지표를 포함해 50개를 검증하며 전체 ingest E2E는 별도 남아 있다.
...
- 실제 frontend owner/admin의 API·위젯 변환 검증 데이터: 오류율 5/15=1/3, 해당 요일·시간대 프롬프트 10건. 고정된 fixture 시각으로 현지 시간 라벨도 대조한다. 기존 지표를 포함해 50개를 검증하며 전체 ingest E2E는 별도 남아 있다.
...
- 실제 frontend owner/admin의 팀·trigger 검증 데이터: 압축 15건, 감소율 4050/5000=0.81. 토큰 쌍이 불완전한 압축 5건은 횟수에 포함하되 감소율에서 제외한다. 기존 지표를 포함해 50개를 검증하며 전체 ingest E2E는 별도 남아 있다.
...
- 실제 frontend owner/admin 클라이언트와 위젯 변환에서 연결 20건, 실패 15/20=0.75 및 서버 속성 라벨을 검증했다. 전체 테스트 888건 통과, 연동 지표 50개이며 ingest 경로 검증은 남아 있다.
...
- 실제 frontend owner/admin의 API 클라이언트와 위젯 변환에서 end_turn 5건·refusal 5건·사유 누락 10건 및 모델 라벨을 확인했다. 전체 테스트 888건과 총 50개 지표 연동 검증이 통과했다. ingest 경로와 전체 수용 검증은 남아 있다.
...
- 실제 frontend owner/admin API·위젯 변환에서 LLM p50/p95/p99=900ms, 턴 p50/p90=1500ms를 확인했다. 전체 테스트 888건과 지표 50개 연동 검증 통과. 첫 토큰 지연은 아래 후속 작업으로 연결했고 전체 ingest E2E는 아직 남아 있다.
...
- 실제 frontend owner/admin에서 사용자 수락 25 / 수락·거절 50 = 0.5 및 kotlin/Edit 라벨을 확인했다. 전체 테스트 888건과 총 50개 지표 연동 검증 통과. 전체 ingest E2E는 별도로 남아 있다.
...
- 실제 frontend owner/admin의 팀별 위젯 변환에서 식별자 2개와 호출 비율 10/15=2/3을 확인했다. 전체 테스트 888건과 총 50개 지표 연동 검증 통과. 전체 ingest E2E는 별도로 남아 있다.
...
- 실제 frontend owner/admin에서 실행 10회·훅 세션 5개·전체 세션 5개·비율 1을 확인했다. 카탈로그 partial을 유지하며 전체 테스트 888건 및 총 50개 지표 연동 검증이 통과했다. 전체 ingest E2E는 별도로 남아 있다.
...
- 실제 frontend owner에서 policy 거부 5건을 확인했다. 공통 48개는 owner/admin 양쪽에서, 모델 거부·벤더 불일치 2개는 owner에서 검증해 총 50개다. 전체 테스트 888건 통과. 전체 ingest E2E는 별도로 남아 있다.
...
- 실제 frontend owner/admin에서 claude-e2e의 사용자 5명을 확인했다. 공통 48개와 owner 전용 모델 거부·벤더 불일치 2개로 총 50개 지표 연동 검증 및 전체 테스트 888건이 통과했다. 이번 frontend 검증 HEAD는 52f7cb10017c6ba6120f51f2e158ff329d14bff0이다. 전체 ingest E2E는 별도로 남아 있다.
...
- 실제 frontend owner/admin에서 캐시 3000/6000=0.5, 입출력 1500/750=2를 확인했다. 공통 48개와 owner 전용 2개로 총 50개 지표 및 전체 테스트 888건 통과. 전체 ingest E2E는 별도로 남아 있다.
...
- 실제 frontend owner/admin에서 이벤트 토큰 input=1500/output=750/cache_read=3000/cache_create=1500, 메트릭의 output+cache_read=250 및 worker 귀속을 확인했다. 공통 48개와 owner 전용 2개로 총 50개 지표 및 전체 테스트 888건 통과. 전체 ingest E2E는 별도로 남아 있다.
...
- 실제 PostgreSQL·ClickHouse에서 마지막 주소 선택, 대소문자·따옴표 주소, 중복 제거, 원천·미매핑 제외, 주소 비노출, 감사·소유자 권한 및 비교 마스킹을 검증했다. 실제 frontend owner에서 불일치 설치 5개와 query 감사 로그 1개를 확인한다. 공통 48개와 owner 전용 2개로 총 50개 지표를 검증하며 전체 ingest E2E는 별도 남아 있다.
...
- 실제 frontend owner/admin에서는 비용 30/15, 활성 사용자 5명, 사용자 활동 1/6시간으로 사용자당 6/3 및 시간당 180/90(공시/계약)을 검증한다. 남은 지표는 cost_anomaly, model_unit_price, contract_commitment_burn이며 전체 ingest E2E도 별도로 남아 있다.
