...
- 기타 재집계 지원 지표는 46개다. 현재 기간 값으로 상위 그룹을 선택하고 비교 기간에 같은 선택을 적용한다. 숨겨진 수치는 순위 선택에 사용하지 않는다.
...
- 서브에이전트 활동: 기간 전체 고유 agent_id로 순위를 정하고 기타의 식별자·도구 호출을 원본에서 중복 제거한다. 실제 도구 호출 구성원으로 소집단을 판단한다. 일반 그룹 상위 N 미연결은 5개이며, group_by 금지 2개는 이 수에 포함하지 않는다. 약정 소진율의 내부 계약별 결과·기간 제한 검토는 별도 유지한다.
