...
`GET /v1/scenarios`와 `GET /v1/scenarios/{scenario_id}`는 로그인한 owner/admin에게 46개 정의를 제공한다. 목록은 `category`, `availability`, `target_page`, `q`를 지원한다. 상세의 `params_schema`는 frontend 폼과 시나리오 서버 입력 검증에서 공통으로 사용한다. S1-1·S1-2·S1-3·S1-4·S1-5·S1-6·S1-7·S2-1·S2-2·S2-3·S3-1·S3-2·S3-3·S3-4·S3-5·S4-1·S4-2·S4-3·S4-4·S4-5·S4-6·S4-8·S5-2·S5-4·S5-5·S5-6·S5-7·S6-1·S6-3·S6-4·S6-5·S7-1·S7-2·S7-3·S7-4·S8-1·S8-2·S8-3·S8-4·S8-5·S8-6·S8-7의 실행 계획과 실행 목록·저장 API를 제공한다. 다른 시나리오의 실행 계획은 후속 작업이다.
...
owner가 감사 사유와 as_of(기본 now), inactive_days(1~365, 기본 30)를 지정한다. 미래 as_of는 거부한다. 최근 해당 경과일의 커버리지·활성 사용자·사용자당 비용과 별도로, 현재 활성 구성원·활성 설치 중 기준 시점 이전에 생성된 설치의 보존 이력을 조회한다.

기준 시점 이상의 이벤트를 제외하고 마지막 관측이 cutoff 이하이면 무활동 집계에 포함한다. 관측 이력이 없으면 생성일을 사용한다. 후보 최대 5,000개이며 초과 시 실패한다. 해당 집계의 구성원이 5명 이상일 때 설치 수만 info로 제공하고 개인·설치 식별자는 응답하지 않는다. 신규·최근 활동·폐기 설치는 제외한다. 과거 상태나 실제 라이선스 회수·절감액을 판단하지 않는다. 보조 지표의 현재 인원·설치 분모는 기존 QRY 정의를 따른다.
