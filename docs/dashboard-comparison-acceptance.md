...
- backend 76e83b4(런타임 86a2a29), frontend 52f7cb1의 E2E에서 실제 S4-4 실행 결과 페이지를 열고 prompts_per_session 위젯의 표를 검사했다. 현재 p50 셀은 `2`, p50_compare 셀은 `미관측`, 행 수는 1개다. 각 헤더와 셀의 열 위치도 확인했다.
- build/e2e/auth-settings/admin-training-comparison-table.png를 직접 확인했다. 현재/이전 값이 나란히 표시되며 이전 미관측을 0으로 바꾸지 않는다. p90도 이미지에서 2/미관측으로 보인다. 자동 단언은 p50 쌍에 적용했다.
- 현재 UI는 metric_id와 p50_compare 같은 원시 필드명을 사용한다. 사용자 친화적인 현재/이전 기간 라벨 개선은 프런트엔드 후속 항목이다. 이번 검증은 S4-4의 현재 값·이전 미관측 사례이며 모든 비교 지표·소집단 마스킹·잔존율의 화면 검증을 대체하지 않는다.

## 프론트엔드 라벨 수정 후

frontend acdc626에서 비교 헤더는 현재 p50/이전 p50로 표시된다. 현재 2와 이전 미관측이 각 열에 유지되는 실제 DOM/E2E와 스크린샷을 확인했다. 과거의 원시 p50_compare 헤더 지적은 해결했다. 모든 비교 지표·소집단 조합의 시각 검증 완료를 뜻하지는 않는다.
