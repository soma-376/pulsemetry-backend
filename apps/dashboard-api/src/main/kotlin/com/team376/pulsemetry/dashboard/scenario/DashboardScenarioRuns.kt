...
import com.team376.pulsemetry.dashboard.catalog.DashboardScenarioCatalog
import com.team376.pulsemetry.dashboard.finding.*
import com.team376.pulsemetry.dashboard.query.DashboardQuery
import com.team376.pulsemetry.dashboard.query.DashboardQueryFilterOverride
import com.team376.pulsemetry.dashboard.query.DashboardQueryFilters
import com.team376.pulsemetry.dashboard.query.DashboardQueryItem
import com.team376.pulsemetry.dashboard.query.DashboardQueryRequest
...
class DashboardScenarioRuns(private val runs: DashboardRuns, private val catalog: DashboardScenarioCatalog,
    private val inputs: DashboardScenarioInputs, private val inactivity: DashboardInactivity, private val probes: DashboardProbeWindows, private val access: DashboardAccess, private val query: DashboardQuery,
    ...
    fun start(id: String, body: JsonNode, user: UserIdentity, audit: String?): DashboardRunRow {
        ...
        val execution = mapper.writeValueAsString(mapOf("tz" to input.zone.id,"price_basis" to input.priceBasis,
            "team_ids" to input.teamIds.map { it.toString() },"organization_scope" to input.organizationScope,"actor_role" to user.role) + (if(id=="S5-4") mapOf("audit_reason" to requireNotNull(input.auditReason)) else emptyMap()) + input.times.filterKeys { it.startsWith("compare_") || it.startsWith("cohort_") }.mapValues { it.value.toString() })
    ...
    private fun runCatalogScenario(row: DashboardRunRow) {
        ...
        val premiumPatterns = if(row.scenario=="S1-2") mapper.readTree(row.params)["premium_model_patterns"].toList().map { it.asString() } else emptyList()
        val frames = linkedMapOf<String,JsonNode>()
        ...
        for ((index, metric) in metricIds.withIndex()) {
            ...
            val request = DashboardQueryRequest(row.from.toString(),row.to.toString(),execution["tz"].asString(),
                ...
                queries=listOf(DashboardQueryItem("A",metric,source=if(row.scenario=="S1-6" && metric=="cost") "metrics" else null,frameType=if(row.scenario=="S1-2") "table" else if(row.scenario=="S8-5" && metric=="active_users") "table" else if(row.scenario=="S1-6" && metric=="cost") "table" else if(row.scenario=="S3-2" && metric=="usage_concentration") "distribution" else if(row.scenario in setOf("S1-1","S3-1","S3-4","S4-6","S8-4") || (row.scenario in setOf("S2-1","S2-3") && metric=="usage_heatmap")) "table" else "timeseries",
                    groupBy=when { row.scenario=="S4-8" && metric=="gate_wait_ms" -> listOf("team"); row.scenario=="S1-2" && metric in setOf("cost","tokens") -> listOf("model"); row.scenario=="S8-5" && metric in setOf("active_users","cost_per_active_user") -> listOf("product"); row.scenario=="S1-6" && metric=="cost" -> listOf("model",if(index==0) "effort" else "speed"); row.scenario in setOf("S2-1","S2-3") && metric=="usage_heatmap" -> listOf("weekday","hour"); row.scenario in setOf("S1-1","S3-1","S3-4") -> listOf("team"); row.scenario=="S4-6" && metric=="tool_calls" -> listOf("action"); row.scenario=="S8-4" -> if(metric=="tokens") listOf("product","model") else listOf("product"); else -> emptyList() },interval="1d",limit=if(row.scenario in setOf("S2-1","S2-3") && metric=="usage_heatmap") null else 100,
                    params=if(row.scenario=="S5-7" && metric=="rubber_stamp_ratio") mapOf("threshold_ms" to mapper.readTree(row.params)["threshold_ms"])
                        else if(row.scenario=="S4-3" && metric=="edit_acceptance_rate" && mapper.readTree(row.params).has("language")) mapOf("language" to mapper.readTree(row.params)["language"])
                        else if(row.scenario=="S4-5" && metric=="command_prompt_ratio" && mapper.readTree(row.params).has("command_names")) mapOf("command_names" to mapper.readTree(row.params)["command_names"]) else emptyMap())))
            // 검증된 사유를 공개 조회 경계에 다시 전달한다. 퍼센트·더하기 문자는 이중 디코딩하지 않는다.
            val queryAudit = if(metric=="vendor_account_mismatch") execution.path("audit_reason").takeIf { it.isString }
                ?.asString()?.let { java.net.URLEncoder.encode(it,java.nio.charset.StandardCharsets.UTF_8) } else null
            val result = mapper.valueToTree<JsonNode>((if(row.scenario=="S1-2" && metric in setOf("cost","tokens"))
                query.premiumScenario(actor(row),request,premiumPatterns) else query.query(actor(row),request,queryAudit,"application/json")).body)["results"]["A"]
            if(result["status"].asInt()!=200) throw UserAuthException(result.path("error").path("error").asString("query_failed"),result["status"].asInt())
        ...
        val thresholdKey = when(row.scenario) { "S1-5" -> "io_ratio_threshold"; "S7-2" -> "density_threshold"; else -> "failure_threshold" }
        val findings = if(row.scenario=="S1-7") {
            if (!runs.progress(row,metricIds.size)) return
            inactivity.findings(actor(row),row.to,mapper.readTree(row.params)["inactive_days"].asInt())
        } else if(row.scenario=="S5-5") {
            if (!runs.progress(row,metricIds.size)) return
            val params = mapper.readTree(row.params)
            probes.findings(actor(row),row.from,row.to,params["probe_window_min"].asInt(),params["probe_count"].asInt())
        } else if(row.scenario=="S1-2") DashboardPremiumModels.findings(frames.getValue("cost"),premiumPatterns)
        else if(row.scenario=="S1-1") DashboardBudgetFindings.evaluate(frames,mapper.readTree(row.params)["budget_by_team"])
            else if(row.scenario=="S2-3") DashboardSprintFindings.evaluate(frames.getValue("sessions"),mapper.readTree(row.params)["sprint_dates"],java.time.ZoneId.of(execution["tz"].asString()))
            else if(row.scenario=="S8-5") DashboardProductUsageFindings.evaluate(frames.getValue("active_users"))
            ...
            else DashboardThresholdFindings.evaluate(row.scenario,frames.getValue(when(row.scenario) { "S5-4" -> "vendor_account_mismatch"; "S6-4" -> "llm_ttft_ms"; "S2-1" -> "rate_limit_events"; "S1-4" -> "cache_read_ratio"; "S3-5" -> "subagent_cost_ratio"; else -> metricIds.first() }),
                when(row.scenario) { "S1-4","S2-1","S2-2","S3-5","S4-2","S4-3","S4-5","S5-2","S5-4","S5-5","S5-6","S5-7","S6-1","S6-3","S6-4","S6-5","S7-3","S7-4","S8-1" -> 0.0; "S4-1" -> 1.0; else -> mapper.readTree(row.params)[thresholdKey].asDouble() },
                details=if(row.scenario=="S4-3" && mapper.readTree(row.params).has("language")) mapOf("language" to mapper.readTree(row.params)["language"].asString()) else if(row.scenario=="S5-7") mapOf("threshold_ms" to mapper.readTree(row.params)["threshold_ms"].asInt()) else emptyMap())
        actor(row)
        if (!runs.progress(row,metricIds.size + if(row.scenario in setOf("S1-7","S5-5")) 1 else 0)) return
        runs.finish(row,mapper.writeValueAsString(mapOf("target_page" to definition["target_page"].asString(),
            ...
            "applied_filters" to mapOf("from" to row.from.toString(),"to" to row.to.toString(),"tz" to execution["tz"].asString(),
                "price_basis" to execution["price_basis"].asString(),"filters" to (mapOf("team_ids" to scope) + if(row.scenario in setOf("S6-1","S6-4")) mapOf("models" to models) else emptyMap())) + if(row.scenario=="S1-2") mapOf(
    ...
    private fun runOnboardingScenario(row: DashboardRunRow, execution: JsonNode, scope: Set<UUID>) {
        ...
        val frames = linkedMapOf<String,JsonNode>()
        val cohort = Instant.parse(execution["cohort_from"].asString()) to Instant.parse(execution["cohort_to"].asString())
        if(!runs.progress(row,0)) return
        val request = DashboardQueryRequest(row.from.toString(),row.to.toString(),execution["tz"].asString(),
            filters=DashboardQueryFilters(teamIds=scope),queries=metrics.mapIndexed { index,metric ->
                DashboardQueryItem(('A'+index).toString(),metric,frameType="table",interval="1d") })
        val results = mapper.valueToTree<JsonNode>(query.onboardingScenario(actor(row),request,cohort).body)["results"]
        for ((index,metric) in metrics.withIndex()) {
            val result = results[('A'+index).toString()]
            if(result["status"].asInt()!=200) throw UserAuthException(result.path("error").path("error").asString("query_failed"),result["status"].asInt())
            frames[metric] = result
        }
        actor(row)
        if(!runs.progress(row,metrics.size)) return
        runs.finish(row,mapper.writeValueAsString(mapOf("target_page" to "P3",
            "highlight_widgets" to listOf("W3.2"),"applied_filters" to mapOf("from" to row.from.toString(),"to" to row.to.toString(),
                "tz" to execution["tz"].asString(),"price_basis" to execution["price_basis"].asString(),"filters" to mapOf("team_ids" to scope),
                "cohort_from" to cohort.first.toString(),"cohort_to" to cohort.second.toString(),"observed_to" to row.to.toString(),
                "limitation" to "선택 팀 범위의 보존 이력에서 처음 관측된 설치를 추적합니다. 보존 이전 사용·실제 신규 가입을 확정하지 않으며 진행 중인 주의 잔존율은 미판정입니다."),
            "frames" to frames,"findings" to emptyList<Any>())),null)
    }
    ...
    private fun runComparisonScenario(row: DashboardRunRow, execution: JsonNode, scope: Set<UUID>) {
        val definition = catalog.detail(row.scenario)
        val models = if(row.scenario=="S6-3") mapper.readTree(row.params).path("models").toList().map { it.asString() }.toSet() else emptySet()
        val before = java.time.Instant.parse(execution["compare_from"].asString()) to
            java.time.Instant.parse(execution["compare_to"].asString())
        val frames = linkedMapOf<String,JsonNode>()
        val metrics = definition["metric_ids"].toList().map { it.asString() }
        for ((index, metric) in metrics.withIndex()) {
            if (!runs.progress(row,index)) return
            val request = DashboardQueryRequest(row.from.toString(),row.to.toString(),execution["tz"].asString(),
                filters=DashboardQueryFilters(teamIds=scope,models=models),priceBasis=execution["price_basis"].asString(),
                queries=listOf(DashboardQueryItem("A",metric,frameType="table",interval="1d",
                    groupBy=if(row.scenario=="S6-3" && metric=="llm_stop_reasons") listOf("stop_reason") else emptyList(),
                    params=if(row.scenario=="S5-6") mapOf("decided_by" to mapper.valueToTree<JsonNode>(listOf("config","hook"))) else emptyMap())))
            val result = mapper.valueToTree<JsonNode>(query.compareScenario(actor(row),request,before).body)["results"]["A"]
            if (result["status"].asInt()!=200) throw UserAuthException(result.path("error").path("error").asString("query_failed"),result["status"].asInt())
            frames[metric] = result
        }
        actor(row)
        if (!runs.progress(row,metrics.size)) return
        val comparison = mapOf("from" to row.from.toString(),"to" to row.to.toString(),
            "compare_from" to before.first.toString(),"compare_to" to before.second.toString(),
            "tz" to execution["tz"].asString())
        runs.finish(row,mapper.writeValueAsString(mapOf("target_page" to definition["target_page"].asString(),
            "highlight_widgets" to definition["highlight_widgets"].toList().map { it.asString() },
            "applied_filters" to (comparison + mapOf("observation_complete" to (row.to <= clock.instant()),"price_basis" to execution["price_basis"].asString(),
                "filters" to (mapOf("team_ids" to scope) + if(row.scenario=="S6-3") mapOf("models" to models) else emptyMap())) +
                if(row.scenario in setOf("S6-3","S8-7")) mapOf("window_weeks" to 4) else emptyMap()), "frames" to frames,
            "findings" to DashboardComparisonFindings.evaluate(row.scenario,frames,comparison,row.to <= clock.instant()))),null)
    }
