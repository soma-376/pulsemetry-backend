...
class DashboardScenarioRuns(private val runs: DashboardRuns, private val catalog: DashboardScenarioCatalog,
    ...
    fun response(row: DashboardRunRow): Map<String,Any?> {
        ...
        "progress" to mapOf("step" to row.step,"total" to when(row.scenario) { "S8-1" -> 7; "S3-1" -> 5; "S4-1","S4-6","S5-2","S6-5","S8-4" -> 2; "S1-1","S1-4","S1-5","S2-1","S2-2","S3-2","S4-2","S4-8","S5-7","S7-3" -> 3; else -> 4 },"label" to when(row.status) { "queued" -> "대기"; "running" -> "지표 조회"; else -> "종료" }),
