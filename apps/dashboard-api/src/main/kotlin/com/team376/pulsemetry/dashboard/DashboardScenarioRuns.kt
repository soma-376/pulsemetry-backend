...
class DashboardScenarioRuns(private val runs: DashboardRuns, private val catalog: DashboardScenarioCatalog,
    ...
    fun start(id: String, body: JsonNode, user: UserIdentity, audit: String?): DashboardRunRow {
        ...
        if (id !in setOf("S1-1","S1-3","S1-4","S1-5","S3-1","S3-4","S4-1","S4-2","S4-6","S4-8","S7-1","S8-4")) throw UserAuthException("scenario_not_implemented",501)
    ...
    fun runOne() {
        ...
        try {
            if (row.scenario in setOf("S1-1","S1-4","S1-5","S3-1","S3-4","S4-1","S4-2","S4-6","S4-8","S7-1","S8-4")) { runCatalogScenario(row); return }
    ...
    private fun runCatalogScenario(row: DashboardRunRow) {
        ...
        val findings = if(row.scenario=="S1-1") DashboardBudgetFindings.evaluate(frames,mapper.readTree(row.params)["budget_by_team"])
            ...
            else DashboardThresholdFindings.evaluate(row.scenario,frames.getValue(if(row.scenario=="S1-4") "cache_read_ratio" else metricIds.first()),
                when(row.scenario) { "S1-4","S4-2" -> 0.0; "S4-1" -> 1.0; else -> mapper.readTree(row.params)[thresholdKey].asDouble() })
        actor(row)
