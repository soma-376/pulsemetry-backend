...
class DashboardScenarioRuns(private val runs: DashboardRuns, private val catalog: DashboardScenarioCatalog,
    ...
    fun start(id: String, body: JsonNode, user: UserIdentity, audit: String?): DashboardRunRow {
        ...
        if (id !in setOf("S1-1","S1-3","S1-4","S1-5","S1-6","S2-1","S2-2","S3-1","S3-2","S3-4","S3-5","S4-1","S4-2","S4-5","S4-6","S4-8","S5-2","S5-7","S6-4","S6-5","S7-1","S7-2","S7-3","S7-4","S8-1","S8-4")) throw UserAuthException("scenario_not_implemented",501)
    ...
    fun response(row: DashboardRunRow): Map<String,Any?> {
        ...
        "progress" to mapOf("step" to row.step,"total" to when(row.scenario) { "S8-1" -> 7; "S3-1" -> 5; "S4-1","S4-5","S4-6","S5-2","S6-5","S8-4" -> 2; "S1-1","S1-4","S1-5","S2-1","S2-2","S3-2","S4-2","S4-8","S5-7","S7-2","S7-3","S7-4" -> 3; else -> 4 },"label" to when(row.status) { "queued" -> "대기"; "running" -> "지표 조회"; else -> "종료" }),
    ...
    fun runOne() {
        ...
        try {
            if (row.scenario in setOf("S1-1","S1-4","S1-5","S1-6","S2-1","S2-2","S3-1","S3-2","S3-4","S3-5","S4-1","S4-2","S4-5","S4-6","S4-8","S5-2","S5-7","S6-4","S6-5","S7-1","S7-2","S7-3","S7-4","S8-1","S8-4")) { runCatalogScenario(row); return }
