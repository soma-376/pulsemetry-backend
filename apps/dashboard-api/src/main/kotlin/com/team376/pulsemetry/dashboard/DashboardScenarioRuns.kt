...
class DashboardScenarioRuns(private val runs: DashboardRuns, private val catalog: DashboardScenarioCatalog,
    ...
    fun start(id: String, body: JsonNode, user: UserIdentity, audit: String?): DashboardRunRow {
        ...
        if (id !in setOf("S1-1","S1-3","S1-4","S1-5","S3-4","S4-1","S4-2","S4-6","S4-8","S7-1")) throw UserAuthException("scenario_not_implemented",501)
    ...
    fun response(row: DashboardRunRow): Map<String,Any?> {
        ...
        "progress" to mapOf("step" to row.step,"total" to when(row.scenario) { "S4-1","S4-6" -> 2; "S1-1","S1-4","S1-5","S4-2","S4-8" -> 3; else -> 4 },"label" to when(row.status) { "queued" -> "대기"; "running" -> "지표 조회"; else -> "종료" }),
    ...
    fun runOne() {
        ...
        try {
            if (row.scenario in setOf("S1-1","S1-4","S1-5","S3-4","S4-1","S4-2","S4-6","S4-8","S7-1")) { runCatalogScenario(row); return }
    ...
    private fun runCatalogScenario(row: DashboardRunRow) {
        ...
        for ((index, metric) in metricIds.withIndex()) {
            ...
            val request = DashboardQueryRequest(row.from.toString(),row.to.toString(),execution["tz"].asString(),
                ...
                queries=listOf(DashboardQueryItem("A",metric,frameType=if(row.scenario in setOf("S1-1","S3-4","S4-6")) "table" else "timeseries",
                    groupBy=when { row.scenario in setOf("S1-1","S3-4") -> listOf("team"); row.scenario=="S4-6" && metric=="tool_calls" -> listOf("action"); else -> emptyList() },interval="1d",limit=100)))
