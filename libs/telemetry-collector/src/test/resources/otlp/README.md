...
2. 재직렬화 왕복 — 신원을 주입하지 않은 모듈 단위 fixture에서 입력과 아카이브를 파싱한 메시지가 같다. 바이트 동일성을 요구하지 않는다
3. 마스킹이 실제 데이터에서 건드리는 것이 없음을 확인한다 — 캡처에 시크릿 패턴이 없다

실제 조립 앱은 마스킹 뒤·아카이브 앞에서 검증된 신원을 스탬핑한다(backend ADR 0016).
따라서 이 fixture의 왕복 검증을 실제 앱의 입력·아카이브 동일성 계약으로 해석하지 않는다.
